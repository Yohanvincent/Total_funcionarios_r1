...
fig.update_layout(
    title="",
    xaxis_title="Horário",
    yaxis_title="Total de Funcionários",
    height=600,
    hovermode="x unified",
...
c1, c2 = st.columns(2)
with c1:
    up_conf = st.file_uploader("Conferentes", ["txt", "csv", "xlsx"], key="total_conf_uploader")
    if up_conf:
...
with c2:
    up_aux = st.file_uploader("Auxiliares", ["txt", "csv", "xlsx"], key="total_aux_uploader")
    if up_aux:

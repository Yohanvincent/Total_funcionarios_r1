...
for x, y in zip(df["Horário"], df["Chegada"]):
    if y != 0:
        anotacoes.append(dict(
            ...
            bordercolor=cor_verde, borderwidth=1,
            bgcolor="white"
        ))

# Saída (agora negativa)
for x, y_real, y_neg in zip(df["Horário"], df["Saída"], df["SaídaNeg"]):
    if y_real != 0:
        anotacoes.append(dict(
            x=x, y=y_neg,
            text=f"-{y_real}",
            ...
            bordercolor=cor_vermelho, borderwidth=1,
            bgcolor="white"
        ))
...
for x, y_real, y_scaled in zip(df["Horário"], df["Equipe"], df["EquipeEscalada"]):
    if y_real > 0:
        anotacoes.append(dict(
            ...
            bordercolor=cor_roxo, borderwidth=1,
            bgcolor="white"
        ))
...
fig.update_layout(
    ...
    height=820,
    barmode="relative",   # <<< AQUI AGORA ESTÁ CERTO!
    hovermode="x unified",
    plot_bgcolor="white",
    paper_bgcolor="white",
    bargap=0.18,

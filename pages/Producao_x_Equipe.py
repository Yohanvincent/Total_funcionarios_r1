...
# exemplo de padrão para conferentes (pode colar o mesmo formato de antes)
padrao_confer = """00:00 04:00 05:15 09:33 9
...
# exemplo de padrão para auxiliares (pode estar vazio por padrão)
...
texto_producao = ler_texto_bytes(st.session_state.prod_bytes, padrao_producao)
texto_confer = ler_texto_bytes(st.session_state.conf_bytes, padrao_confer)
texto_aux = ler_texto_bytes(st.session_state.aux_bytes, padrao_aux)
...
# --- Leitura Jornadas (conferentes / auxiliares) ---
def jornadas(t):
    j = []
    for l in t.strip().split("\n"):
        ...
        if not p:
            continue
        # formato com intervalo: 5 colunas e última é QTD
        if len(p) == 5 and p[4].isdigit():
            j.append({"t": "c", "e": p[0], "si": p[1], "ri": p[2], "sf": p[3], "q": int(p[4])})
        # formato simples: inicio, fim, qtd
...
# coletar jornadas separadas
jornadas_conf = jornadas(texto_confer)
jornadas_aux = jornadas(texto_aux)

# Todos os horarios - inclui horários de produção (cheg/said) e de jornadas
...
def calcular_equipe(jornadas_list, horarios):
    ...
    for j in jornadas_list:
        e = min_hora(j["e"])
        # se entrada com intervalo
        if j["t"] == "c":
            si = min_hora(j["si"])
            ri = min_hora(j["ri"])
            sf = min_hora(j["sf"])
            for i, t in enumerate(tl):
                # assume: trabalho antes do intervalo (e <= t < si) e após o intervalo (ri <= t <= sf)
...
# calcular equipe combinando conferentes + auxiliares
...
# Escala para desenhar a equipe na mesma faixa do eixo de toneladas
...
# Linha da equipe — usamos customdata para mostrar o valor real no hover (não o valor escalado)
    ...
    for _, r in df.iterrows():
        ...
        if r["Saida_Ton"] > 0:
            fig.add_annotation(x=r["Horario"], y=r["Saida_Ton"],
                ...
                showarrow=False, yshift=10)
        if r["Equipe"] > 0:
            # anotação com o número real de funcionários (não a escala)
...
# --- Uploads em colunas: Producao / Conferente / Auxiliar ---
...
if st.session_state.prod_name or st.session_state.conf_name or st.session_state.aux_name:

...
st.title("📦 Capacidade × Produção × Acumulado × Funcionários (toneladas)")
...
def min_hora(h):
    try:
        return sum(int(x) * (60 ** i) for i, x in enumerate(reversed(h.split(":"))))
    except:
        return 0
...
with cols[0]:
    ...
    if up:
        st.session_state.chegadas_bytes, st.session_state.chegadas_name = up.getvalue(), up.name
    if st.session_state.chegadas_name:
        st.success(st.session_state.chegadas_name)
with cols[1]:
    up = st.file_uploader("Saídas", ["txt","csv","xlsx"], key="s")
    if up:
        st.session_state.saidas_bytes, st.session_state.saidas_name = up.getvalue(), up.name
    if st.session_state.saidas_name:
        st.success(st.session_state.saidas_name)
with cols[2]:
    up = st.file_uploader("Conferentes", ["txt","csv","xlsx"], key="conf")
    if up:
        st.session_state.conf_bytes, st.session_state.conf_name = up.getvalue(), up.name
    if st.session_state.conf_name:
        st.success(st.session_state.conf_name)
with cols[3]:
    up = st.file_uploader("Auxiliares", ["txt","csv","xlsx"], key="aux")
    if up:
        st.session_state.aux_bytes, st.session_state.aux_name = up.getvalue(), up.name
    if st.session_state.aux_name:
        st.success(st.session_state.aux_name)
with cols[4]:
    up = st.file_uploader("Capacidade", ["txt","csv","xlsx"], key="cap")
    if up:
        st.session_state.capacidade_bytes, st.session_state.capacidade_name = up.getvalue(), up.name
    if st.session_state.capacidade_name:
        st.success(st.session_state.capacidade_name)
...
with st.expander("Tabela completa (toneladas)"):
    ...
    st.download_button(
        "Baixar CSV (toneladas)",
        csv,
        "dados_unificados_ton.csv",
        "text/csv"
    )

...
def extrair_producao(texto):
    ...
    for l in texto.strip().split("\n"):
        ...
        if len(p) >= 2:
            try:
                v = float(p[1].replace(",", "."))
                if modo == "cheg": cheg[p[0]] = cheg.get(p[0], 0) + v
                else: said[p[0]] = said.get(p[0], 0) + v
            except:
                pass
    return cheg, said
...
horarios = sorted(set(cheg.keys()) | set(said.keys()), key=min_hora)
...
y_max = max(max(cheg_val), max(said_val)) + 5
eq_max = max(eq_total) + 5
scale = y_max / eq_max
df["Equipe_Escalada"] = df["Equipe"] * scale

# =====================================================================
# GRÁFICO AJUSTADO (visual igual ao exemplo original)
# =====================================================================
...
# Linha roxa mais suave + tracejado delicado
fig.add_trace(go.Scatter(
    x=df["Horario"], y=df["Equipe_Escalada"],
    mode="lines+markers",
    name="Equipe",
    line=dict(color="#9B59B6", width=3, dash="dot"),  # pontos menores e mais leves
    marker=dict(size=6),
...
# =====================================================================
# RÓTULOS COM CAIXINHAS (exatamente como no seu código original)
# =====================================================================
...
fig.update_layout(
    xaxis_title="Horário",
    yaxis=dict(title="Toneladas | Equipe (escalada)", range=[0, y_max], zeroline=False),
    height=650,
    hovermode="x unified",

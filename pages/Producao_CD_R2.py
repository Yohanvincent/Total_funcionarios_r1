...
# Saídas (TON NEGATIVA MAS BARRA PRA CIMA)
saidas = {}
...
fig.add_trace(go.Bar(
    x=df["Horário"], y=df["Chegada"],
    name="Chegada",
    marker_color="#90EE90",
    opacity=0.9
))

# Saída (vermelho, MAS mostrando como POSITIVO no gráfico)
fig.add_trace(go.Bar(
    x=df["Horário"],
    y=df["Saída"],           # VALOR POSITIVO
    name="Saída",
    marker_color="#E74C3C",
    opacity=0.9
))
...
if st.session_state.rotulos:
    for _, r in df.iterrows():
        ...
        # Saída (NEGATIVO no texto)
        if r["Saída"] > 0:
            fig.add_annotation(
                x=r["Horário"], y=r["Saída"],
                text=f"-{r['Saída']}",
        ...
        if r["Equipe"] > 0:
            fig.add_annotation(
                x=r["Horário"], y=r["EquipeEscalada"],
                text=f"{int(r['Equipe'])}",

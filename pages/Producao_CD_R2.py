...
st.title("Produção × Equipe CD – Visualização Final")
...
# ================= GRÁFICO FINAL – COLUNAS EMPILHADAS + RÓTULOS PEQUENOS =================
fig = go.Figure()

# Chegada (verde) e Saída (vermelho) empilhadas no mesmo horário
fig.add_trace(go.Bar(
    x=df["Horário"],
    y=df["Chegada (ton)"],
    name="Chegada",
    marker_color="#90EE90",
    text=df["Chegada (ton)"].apply(lambda x: f"+{x}" if x > 0 else ""),
    textposition="inside",
...
fig.add_trace(go.Bar(
    ...
    marker_color="#E74C3C",
    text=df["Saída (ton)"].apply(lambda x: f"-{x}" if x > 0 else ""),
    textposition="inside",
    insidetextanchor="middle",
...
fig.update_layout(
    title="Produção × Equipe Total – Chegada (verde) | Saída (vermelho acima) | Equipe (linha roxa)",
    xaxis_title="Horário",
    yaxis=dict(title="Toneladas", range=[0, max_ton * 1.3]),
    height=800,
    barmode="stack",  # ← AQUI: colunas empilhadas!
    hovermode="x unified",
    legend=dict(orientation="h", yanchor="top", y=-0.15, xanchor="center", x=0.5),
    margin=dict(l=70, r=70, t=100, b=140)
)
...
if "00:00" in df["Horário"].values:
    ...
    st.success(f"Às 00:00 → **{eq} funcionários** (125 com seus dados)")
...
st.session_state.rotulos = st.checkbox("Rótulos dentro das barras", value=True)

st.success("PRONTO! Colunas empilhadas, saída em vermelho, rótulos pequenos e limpos. Visualização perfeita! 28/11/2025")
